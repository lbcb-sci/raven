...
static std::uint32_t removeBubbles(Graph& graph) {
    ...
    auto path_extract = [&] (Graph::Node* begin, Graph::Node* end) -> std::vector<Graph::Node*> {
    ...
    auto path_sequence = [] (const std::vector<Graph::Node*>& path) -> std::string {
        std::string data{};
        for (std::uint32_t i = 0; i < path.size() - 1; ++i) {
    ...
    auto bubble_pop = [&] (
            const std::vector<Graph::Node*>& lhs,
            const std::vector<Graph::Node*>& rhs) -> std::unordered_set<std::uint32_t> {
...
static void createForceDirectedLayout(std::shared_ptr<thread_pool::ThreadPool>& threadPool, const Graph& graph, const std::string& path = "") {
    std::ofstream os;
    bool is_first = true;
    if (!path.empty()) {
        os.open(path);
...
static std::uint32_t removeLongEdges(std::shared_ptr<thread_pool::ThreadPool>& threadPool, Graph& graph, std::uint32_t num_rounds) {
...
static std::uint32_t salvagePlasmids(std::shared_ptr<thread_pool::ThreadPool>& threadPool, Graph& graph) {
    createUnitigs(graph);
...
static void removeLongEdgesStage(Graph& graph, std::shared_ptr<thread_pool::ThreadPool>& threadPool) {
    biosoup::Timer timer;
    timer.Start();
...
static void stageExecution(Graph& graph, bool checkpoints, Fun fun, Args&... args) {
