...
bool overlapUpdate(biosoup::Overlap& o, const std::vector<std::unique_ptr<Pile>>& piles) {

    if (piles[o.lhs_id]->is_invalid() ||
        piles[o.rhs_id]->is_invalid()) {
        return false;
    }
    ...
    std::uint32_t lhs_begin = o.lhs_begin + (o.strand ?
                                             (o.rhs_begin < piles[o.rhs_id]->begin() ?
                                              piles[o.rhs_id]->begin() - o.rhs_begin : 0)
                                                      :
                                             (o.rhs_end > piles[o.rhs_id]->end() ?
                                              o.rhs_end - piles[o.rhs_id]->end() : 0));
    ...
    std::uint32_t rhs_begin = o.rhs_begin + (o.strand ?
                                             (o.lhs_begin < piles[o.lhs_id]->begin() ?
                                              piles[o.lhs_id]->begin() - o.lhs_begin : 0)
                                                      :
                                             (o.lhs_end > piles[o.lhs_id]->end() ?
                                              o.lhs_end - piles[o.lhs_id]->end() : 0));
...
std::uint32_t getOverlapType(const biosoup::Overlap& o, const std::vector<std::unique_ptr<Pile>>& piles) {

    std::uint32_t lhs_length =
            piles[o.lhs_id]->end() - piles[o.lhs_id]->begin();
    ...
    std::uint32_t rhs_length =
            piles[o.rhs_id]->end() - piles[o.rhs_id]->begin();
...
bool overlapFinalize(biosoup::Overlap& o, const std::vector<std::unique_ptr<Pile>>& piles) {
...
std::vector<std::vector<std::uint32_t>> connectedComponents(const std::vector<std::vector<biosoup::Overlap>>& overlaps,
