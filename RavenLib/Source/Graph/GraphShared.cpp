...
void removeEdges(Graph& graph, const std::unordered_set<std::uint32_t>& indices, bool remove_nodes) {

    auto erase_remove = [] (std::vector<Graph::Edge*>& edges, Graph::Edge* marked) -> void {
...
std::uint32_t createUnitigs(Graph& graph, std::uint32_t epsilon) {
    ...
    for (const auto& it : graph.nodes) {
        ...
        while (true) {
            ...
            unitig->transitive.insert(
                    graph.nodes[jt->id & ~1UL]->transitive.begin(),
    ...
    graph.edges.insert(graph.edges.end(), unitig_edges.begin(), unitig_edges.end());
...
std::vector<std::unique_ptr<biosoup::NucleicAcid>> getUnitigs(Graph& graph, bool drop_unpolished) {
