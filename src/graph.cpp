...
void Graph::Construct(std::vector<std::unique_ptr<biosoup::Sequence>>& sequences) {
  if (sequences.empty()) {
    return;
  }
  ...
  if (true) {  // trim and annotate piles
    timer.Start();
  ...
  if (true) {  // resolve contained reads
    timer.Start();
    ...
    for (std::uint32_t i = 0; i < overlaps.size(); ++i) {
      ...
      for (std::uint32_t j = 0; j < overlaps[i].size(); ++j) {
        ...
        if (type == 1 &&
            !piles_[overlaps[i][j].rhs_id]->has_chimeric_region()) {
          piles_[i]->set_contained();
        } else if (type == 2 &&
            !piles_[i]->has_chimeric_region()) {
          piles_[overlaps[i][j].rhs_id]->set_contained();
        } else {
  ...
  if (true) {  // resolve chimeric sequences
    timer.Start();
      ...
      if (!is_changed) {
        for (const auto& it : overlaps) {
          for (const auto& jt : it) {
            ...
            if (type == 1) {
              piles_[jt.lhs_id]->set_contained();
              piles_[jt.lhs_id]->set_invalid();
            } else if (type == 2) {
              piles_[jt.rhs_id]->set_contained();
              piles_[jt.rhs_id]->set_invalid();
            }
          }
        }
        overlaps.clear();
  ...
  if (true) {  // clear piles for sensitive overlaps
    timer.Start();
  ...
  // update piles with repetitive regions
  ...
  while (true) {
    ...
    for (const auto& it : overlaps.back()) {
      piles_[it.lhs_id]->resolve_repetitive_regions(it);
      piles_[it.rhs_id]->resolve_repetitive_regions(it);
    }
    ...
    for (std::uint32_t i = 0; i < overlaps.back().size(); ++i) {
      ...
      if (piles_[it.lhs_id]->is_false_overlap(it) ||
          piles_[it.rhs_id]->is_false_overlap(it)) {
        is_changed = true;
      } else {
        overlaps.back()[j++] = it;
      }
  ...
  for (const auto& it : piles_) {
    ...
    const auto& data = sequences[it->id()]->data.substr(
        it->begin(),
        it->end() - it->begin());

    std::unique_ptr<Node> n(new Node(node_id++, it->id(), name, data));
    std::unique_ptr<Node> nc(new Node(
        node_id++,
        it->id(),
        name,
  ...
  std::cerr << "[raven::Graph::Construct] stored " << nodes_.size() << " nodes "
            << std::fixed << timer.Stop() << "s"
            << std::endl;
  ...
  for (const auto& it : overlaps.back()) {
    Node* lhs = nodes_[sequence_to_node[it.lhs_id]].get();
    Node* rhs = nodes_[sequence_to_node[it.rhs_id] + 1 - it.strand].get();

    std::uint32_t lhs_length =
        piles_[it.lhs_id]->end() - piles_[it.lhs_id]->begin();
    std::uint32_t lhs_begin = it.lhs_begin - piles_[it.lhs_id]->begin();
    std::uint32_t lhs_end = it.lhs_end - piles_[it.lhs_id]->begin();

    std::uint32_t rhs_length =
        piles_[it.rhs_id]->end() - piles_[it.rhs_id]->begin();
    std::uint32_t rhs_begin = it.strand ?
        it.rhs_begin - piles_[it.rhs_id]->begin() :
        rhs_length - (it.rhs_end - piles_[it.rhs_id]->begin());
    std::uint32_t rhs_end = it.strand ?
        it.rhs_end - piles_[it.rhs_id]->begin():
        rhs_length - (it.rhs_begin - piles_[it.rhs_id]->begin());
    ...
    if (type == 3) {
      std::unique_ptr<Edge> e(new Edge(
          ...
          rhs,
          lhs_begin - rhs_begin));
      std::unique_ptr<Edge> ec(new Edge(
          edge_id++,
          rhs->pair,
          lhs->pair,
          (rhs_length - rhs_end) - (lhs_length - lhs_end)));
      ...
      lhs->outedges.emplace_back(e.get());
      lhs->pair->inedges.emplace_back(ec.get());
      rhs->inedges.emplace_back(e.get());
      rhs->pair->outedges.emplace_back(ec.get());
      ...
      rhs->outedges.emplace_back(e.get());
      rhs->pair->inedges.emplace_back(ec.get());
      lhs->inedges.emplace_back(e.get());
      lhs->pair->outedges.emplace_back(ec.get());
  ...
  std::cerr << "[raven::Graph::Construct] stored " << edges_.size() << " edges "
            << std::fixed << timer.Stop() << "s"
            << std::endl;
...
std::uint32_t Graph::RemoveBubbles() {
  ...
  auto bubble_type = [&] (
    ...
    auto other_sequence = std::unique_ptr<biosoup::Sequence>(new biosoup::Sequence());  // NOLINT
    ...
    return matches > 0.5 * std::min(sequence->data.size(), other_sequence->data.size());  // NOLINT
  };
    ...
    if (found_sink) {
      ...
      if (bubble_type(path, other_path)) {
        ...
        if (path_num_reads > other_path_num_reads) {
          FindRemovableEdges(edges_for_removal, other_path);
          if (edges_for_removal.empty()) {
            FindRemovableEdges(edges_for_removal, path);
          }
        } else {
          FindRemovableEdges(edges_for_removal, path);
          if (edges_for_removal.empty()) {
...
std::uint32_t Graph::RemoveLongEdges() {
...
void Graph::Polish(const std::vector<std::unique_ptr<biosoup::Sequence>>& sequences,
  ...
  auto polisher = racon::Polisher::Create(q, 0.3, 500, true, match, mismatch,
  ...
  GetUnitigs(unitigs);  // TODO(rvaser): set id to node->id
...
void Graph::GetUnitigs(std::vector<std::unique_ptr<biosoup::Sequence>>& dst,
  ...
  for (const auto& it : nodes_) {
    ...
    dst.emplace_back(std::unique_ptr<biosoup::Sequence>(new biosoup::Sequence(
        name, it->data)));
  }
}

void Graph::RemoveMarkedObjects(bool remove_nodes) {
...
void Graph::FindRemovableEdges(std::vector<std::uint32_t>& dst,
    const std::vector<std::uint32_t>& path) {
  ...
  auto find_edge = [&] (std::uint32_t src, std::uint32_t dst) -> std::uint32_t {
    ...
    if (!found_edge) {
      throw std::logic_error("[raven::Graph::FindRemovableEdges] error: "
